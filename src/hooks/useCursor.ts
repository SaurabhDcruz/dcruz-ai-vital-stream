  ...
  useEffect(() => {
    ...
    const updateLoop = () => {
      ...
      if (cursorRef.current) {
        ...
        cursorRef.current.style.backgroundColor = isClickedNow
          ? 'rgba(59, 130, 246, 0.4)'
          : 'rgba(255,255,255,0.1)';
        cursorRef.current.style.scale = isClickedNow
          ? '0.7'
          : gestureTypeNow !== 'none'
            ? '1.5'
            : '1';
        cursorRef.current.style.borderColor =
          gestureTypeNow === 'none' ? 'rgba(59, 130, 246, 0.2)' : 'rgba(59, 130, 246, 1)';
      }
