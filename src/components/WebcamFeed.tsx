 ...
 */
export const WebcamFeed: React.FC<WebcamFeedProps> = ({ onStreamReady, onFrame, className, minimal }) => {
  ...
  const startWebcam = async () => {
    ...
    try {
      ...
      if (errorName === 'NotAllowedError' || errorName === 'PermissionDeniedError' || err.message?.includes('denied')) {
        let msg = 'Camera access was denied.';
        if (isIframe) {
          msg = 'Camera access is blocked by the preview environment. This is a common security restriction for embedded apps.';
        }
        setError(msg);
        ...
        setError('Security restriction: The browser blocked camera access for this embedded frame.');
      } else {
  ...
  const content = (
    <div ref={containerRef} className={`relative bg-zinc-950 aspect-video flex items-center justify-center overflow-hidden ${minimal ? 'w-full h-full' : ''}`}>
      <AnimatePresence mode="wait">
        {status === 'idle' && (
          <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 flex flex-col items-center justify-center gap-3 z-20 bg-zinc-950/90 backdrop-blur-md p-4 text-center">
            ...
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-zinc-400 max-w-[180px] mx-auto">Gesture control requires webcam access to track hand movements.</p>
            </div>
            <Button onClick={startWebcam} size="sm" className="bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-lg px-6 shadow-lg shadow-blue-500/20">
              Enable Camera
            </Button>
          </motion.div>
        )}
        ...
        {status === 'loading' && (
          <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 flex flex-col items-center justify-center gap-4 z-20 bg-zinc-950/50 backdrop-blur-sm">
            <RefreshCw className="w-8 h-8 text-blue-500 animate-spin" />
            <p className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest">Requesting Access...</p>
          </motion.div>
        )}

        {status === 'error' && (
          <motion.div key="error" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center z-20 bg-zinc-950/90 backdrop-blur-md">
            <div className={`${minimal ? 'p-4' : 'p-8'} bg-white rounded-2xl border border-zinc-200 shadow-2xl max-w-sm w-full`}>
              ...
              <div className="flex flex-col gap-2">
                <Button onClick={startWebcam} size="sm" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl">
              ...
              <div className="mt-4 pt-4 border-t border-zinc-100">
                <p className="text-[9px] text-zinc-400 font-medium uppercase tracking-wider mb-2">Troubleshooting</p>
  ...
  return (
    <Card className={`overflow-hidden border-zinc-200 shadow-xl bg-white/50 backdrop-blur-sm ${className}`}>
